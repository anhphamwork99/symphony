...
| T02-AC1 execution record + executionId durable before child-start evidence | 20 | AC seq-1-before-start, REP atomic admission, INT stage 3 (journal [1,2] before completion evidence) | `bun run test src/provider/piSubagentAdmissionCoordinator.test.ts` + INT | pass |
...
3. Focused deterministic suites, exit 0: piSubagentProgressObservation 4/4;
   ...
   main.test green.
4. Migration checks: `node scripts/check-migration-lineage.ts` exit 0;
