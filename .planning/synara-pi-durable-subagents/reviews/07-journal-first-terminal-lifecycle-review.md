...
- **Boundary (per packet):** completion *delivery* (outbox/follow-up turns) is Tickets 08–09 — only the post-commit seam (`onTerminalPersisted` / `subagents/terminal-settled`) is in scope. Restart reconciliation is Ticket 10; sequence-gap *repair* is out of scope (diagnostic only).
