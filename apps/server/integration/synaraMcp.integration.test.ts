...
const startSessionTurn = async (
  ...
  await Effect.runPromise(
    harness.adapterHarness.queueTurnResponseForNextSession(EMPTY_TURN_RESPONSE),
  );
  await client.dispatchCommand({
...
const journalHasSlashMessage = async (
  ...
  return events.some(
    (event) =>
      event.type === "thread.message-sent" && (event.payload as { text?: unknown }).text === text,
  );
};
...
// Journey 2 (WP4 slice)
// ---------------------------------------------------------------------------
it("WP4 journey: disable DURING an active turn with an in-flight MCP call, fence/settle ordering, structured exactly-once settlement, turn continuity (no interrupt, no replay), duplicate idempotency", async () => {
  ...
  try {
    ...
    await waitFor(
      "in-flight MCP call settled as disabled",
...
// Journey 3 (WP5 slice)
// ---------------------------------------------------------------------------
it("WP5 journey: future-session waiting, failed-sibling global rollback with sibling cleanup, and restart recovery with exactly-once terminal and zero provider replay", async () => {
  ...
  try {
    ...
    await dispatchMcpCommand(harness1, threadE, rollbackEnableCommandId, SYNARA_MCP_ENABLE_COMMAND);
    ...
    expect(
      rollbackOperation.outcomes.map((outcome) => [outcome.sessionId, outcome.status]),
    ).toEqual([
    ...
    const rollbackTerminalThread = await harness1.waitForThread(threadE, (thread) =>
      thread.activities.some(
        (activity) =>
    ...
    await waitFor(
      "deferred retry enable call on thread E",
    ...
    expect(
      pendingWithFuture.synaraMcpActivationOperation!.waitSet.map((member) => member.sessionId),
    ).toEqual([String(threadE), String(threadF)]);
    ...
    await waitFor(
      "future session G converged to active",
      () => harness1.adapterHarness.getEnableCalls(threadG).length === 1,
    );
    const gEnableCalls = harness1.adapterHarness.getEnableCalls(threadG);
    expect(gEnableCalls[0]!.expectedSessionGeneration).toBe(gEnableCalls[0]!.liveSessionGeneration);
    ...
    await dispatchMcpCommand(harness1, threadH, recoveryEnableCommandId, SYNARA_MCP_ENABLE_COMMAND);
    await harness1.waitForProject(
      projectRecovery,
      (project) =>
    ...
    await waitFor(
      "deferred recovery enable call on thread H",
    ...
    const recoveredTerminalThread = await harness2.waitForThread(threadH, (thread) =>
      thread.activities.some(
        (activity) =>
    ...
    expect(String((recoveredTerminals[0]!.payload as { detail?: string }).detail)).toContain(
...
// Journey 1 (WP3 slice)
// ---------------------------------------------------------------------------
it("WP3 journey: dormant startup, multi-session enable with pending/terminal, generation-fenced failure rollback, reconnect/replay equivalence, zero contamination", async () => {
  const harness = await makeWsOrchestrationHarness({ provider: "codex" });
  try {
    ...
    await waitFor(
      "deferred enable call on thread A",
    ...
    expect(harness.adapterHarness.getDisableCalls(threadB)).toEqual([{ stages: ["fence"] }]);
    ...
    expect(
      activityIdsOf(terminalThread).filter((id) => id === `${requestId}:pending`),
    ).toHaveLength(1);
...
// ---------------------------------------------------------------------------
it("WP3b journey: successful enable terminal exactly once, dormant and authority fail-closed MCP admission, and fresh subject-bound authority after enabled reconnect", async () => {
  const harness = await makeWsOrchestrationHarness({ provider: "codex" });
  ...
  try {
    ...
    await waitFor(
      "fresh session C converged to active",
